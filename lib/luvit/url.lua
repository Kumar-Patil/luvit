...
local function parseLatest(url, parseQueryString)
  ...
  if parseQueryString then
    query = querystring.parseLatest(query)
  end
...
local function resolveObject(source, relative)
  source = URL.parseLatest(source)
  relative = URL.parseLatest(relative)
